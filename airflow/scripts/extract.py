...
async def fetch_data(session, year):
    ...
    url = f"https://historical-forecast-api.open-meteo.com/v1/forecast?latitude=52.52&longitude=13.41&start_date={year}-01-01&end_date={year}-01-31&hourly=temperature_2m,relative_humidity_2m,dew_point_2m,precipitation_probability,precipitation,rain,pressure_msl,surface_pressure,cloud_cover,wind_speed_10m,wind_direction_10m,soil_temperature_6cm,soil_temperature_18cm,soil_moisture_0_to_1cm,soil_moisture_3_to_9cm,is_day,sunshine_duration,direct_radiation,diffuse_radiation&daily=sunrise,sunset,daylight_duration&timezone=Asia%2FBangkok"
    try:
...
async def main():
    async with aiohttp.ClientSession() as session:  # Creates a session for all requests
        tasks = [fetch_data(session, year) for year in range(1967, 2017 + 1)]
...
def save_to_postgres(df):
    ...
    df.to_sql("solar_crops_raw", engine, if_exists="replace", index=False)
